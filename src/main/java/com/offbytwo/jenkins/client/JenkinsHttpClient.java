...
public class JenkinsHttpClient {
	...
	public <R extends BaseModel, D> R post(String path, D data, Class<R> cls)
		...
		if (crumb != null) {
			request.addHeader(new BasicHeader(crumb.getCrumbRequestField(),
					crumb.getCrumb()));
		}
	...
	public String post_xml(String path, String xml_data, boolean crumbFlag)
			throws IOException {
		HttpPost request = new HttpPost(api(path));
		if (crumbFlag == true) {
			Crumb crumb = get("/crumbIssuer", Crumb.class);
